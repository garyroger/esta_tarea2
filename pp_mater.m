...
nivel_h = 0.368 * h;  % nivel de cruce
% buscar tiempo en el que i_fd cruza ese nivel
win_vf_sin = (t >= t0) & (t <= t0 + 0.2);
[~, idx_local] = min(abs(vt_sen(win_vf_sin) - nivel_h));
idx1 = find(win_vf_sin, 1, 'first') + idx_local - 1;
t_cruce_h = t(idx1);
Tpp = t_cruce_h - t0;  % T''qo
% ===== Mostrar resultados =====
